...
import { Rocket, Calendar, FileText, Tag, Hash, Eye, Lock, Clock, CheckCircle, Loader, AlertTriangle } from 'lucide-react';
...
export const WordPressPublishPanel: React.FC<WordPressPublishPanelProps> = ({
  topic,
  sections,
  metadata,
  onSuccess
  ...
  if (success) {
    return (
      <div className="bg-emerald-50 rounded-3xl p-8 border-2 border-emerald-200 text-center">
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex gap-1">
              <span className="px-3 py-3 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 text-xs font-mono whitespace-nowrap">
                /.../
              </span>
              <input
                type="text"
              ...
              {statusOptions.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-xl border-2 transition-all text-right ${options.status === opt.value
                        ? `${getStatusColor(opt.color)} border-current shadow-lg scale-[1.02]`
                        : 'bg-white border-slate-200 hover:border-slate-300'
                      }`}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <Icon className={`w-4 h-4 ${options.status === opt.value ? 'text-current' : 'text-slate-400'}`} />
                      <span className={`font-bold text-xs ${options.status === opt.value ? 'text-current' : 'text-slate-600'}`}>
                        {opt.label}
                      </span>
                    </div>
                  </button>
          ...
          {options.status === 'future' && (
            <div className="p-4 bg-blue-50 rounded-xl border border-blue-200">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-bold text-blue-800 text-xs">توقيت النشر المجدول</span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[10px] font-bold text-blue-700 mb-1">التاريخ</label>
                  <input
                    type="date"
                    value={options.scheduledDate}
                    onChange={(e) => setOptions({ ...options, scheduledDate: e.target.value })}
                    min={new Date().toISOString().split('T')[0]}
                    className="w-full px-2 py-2 border border-blue-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-blue-700 mb-1">الوقت</label>
                  <input
                    type="time"
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl font-bold text-xs transition-all ${options.commentStatus === 'open'
              ...
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl font-bold text-xs transition-all ${options.commentStatus === 'closed'
          ...
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-blue-200 flex items-center justify-center gap-2"
          >
            {isPublishing ? (
              <>
